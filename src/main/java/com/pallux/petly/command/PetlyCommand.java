...
public class PetlyCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command,
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "achievements" -> {
                if (args.length < 3 || !args[1].equalsIgnoreCase("check")) { sendHelp(sender); return true; }
                ...
                PlayerData aData = plugin.getPlayerDataManager().get(target.getUniqueId());
                plugin.getAchievementSystem().checkAll(aData);
                sender.sendMessage(TextUtil.parse("<green>Rechecked achievements for <white>" + args[2] + "<green>."));
            }
    ...
    private void handleReset(CommandSender sender, String playerName, String type) {
        ...
        switch (type.toLowerCase()) {
            ...
            case "achievements" -> {
                ...
                data.getClaimedAchievements().clear();
            }
            default -> {
                sender.sendMessage(TextUtil.parse("<red>Unknown type. Valid: all, dust, pets, petxp, petlevel, thetower, fieldmissions, milestones, quests, achievements"));
                return;
            }
        }
        plugin.getPlayerDataManager().saveAsync(target.getUniqueId());
    ...
    private void sendHelp(CommandSender sender) {
        sender.sendMessage(TextUtil.parse(
                ...
                "/petly achievements check <player>\n" +
                "/petly reset <player> <all|dust|pets|petxp|petlevel|thetower|fieldmissions|milestones|quests|achievements>"
        ));
    }
